...
namespace ServidorTresEnRayaForm
{
    public partial class Jugador2 : Form
    {
        ...
        private void Jugador2_Load(object sender, EventArgs e)
        {
            Gato = new ReconfigurarCuadro[3, 3];//da posiciones al tablero
            Gato[0, 0] = new ReconfigurarCuadro(tablero0Panel2, ' ', 0);
            Gato[0, 1] = new ReconfigurarCuadro(tablero1Panel2, ' ', 1);
            Gato[0, 2] = new ReconfigurarCuadro(tablero2Panel2, ' ', 2);
            Gato[1, 0] = new ReconfigurarCuadro(tablero3Panel2, ' ', 3);
            Gato[1, 1] = new ReconfigurarCuadro(tablero4Panel2, ' ', 4);
            Gato[1, 2] = new ReconfigurarCuadro(tablero5Panel2, ' ', 5);
            Gato[2, 0] = new ReconfigurarCuadro(tablero6Panel2, ' ', 6);
            Gato[2, 1] = new ReconfigurarCuadro(tablero7Panel2, ' ', 7);
            Gato[2, 2] = new ReconfigurarCuadro(tablero8Panel2, ' ', 8);
        ...
        public void PintarCuadros() // dibuja para paneles
        {
            ...
            for (int fila = 0; fila < 3; fila++)
            {
                for (int columna = 0; columna < 3; columna++)
                {
                    ...
                    g.DrawString(Gato[fila, columna].Marca.ToString(), tablero0Panel2.Font, brocha, 10, 8);// dibuja X o O con brocha
                }
            }
        }
            ...
            for (int fila = 0; fila < 3; fila++)
            {
                for (int columna = 0; columna < 3; columna++)
                {
                    if (Gato[fila, columna].PanelCuadros == sender)
                    {
                        ...
                        EnviarCuadradoClic(Gato[fila, columna].Ubicacion);//movimiento al servidor y cambiar turno
                    }
                }
            }
        }
